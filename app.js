...
const DB_VERSION = 2; // <<-- MODIFICATO: Versione incrementata per aggiornare la struttura
const DATA_STORE_NAME = 'appData';
const FORM_STORE_NAME = 'formState';
const PROFILES_STORE_NAME = 'clientProfiles'; // <<-- NUOVO: Store per i profili cliente
...
async function openDB() {
    ...
    db = await idb.openDB(DB_NAME, DB_VERSION, {
        upgrade(db) {
            ...
            // <<-- NUOVO: Crea lo store per i profili cliente se non esiste
...
async function renderApp(data) {
    ...
    appContent.innerHTML = `
        ...
        <div id="client-profile-section" class="bg-gray-100 p-4 rounded-lg mb-8 border border-gray-200 shadow-sm">
            <h2 class="text-lg font-bold mb-3 text-center text-gray-700">Gestione Clienti</h2>
            <div class="flex items-center gap-2 md:gap-4 flex-wrap justify-center">
                <select id="client-selector" class="flex-grow min-w-[200px]">
                    <option value="">Seleziona un cliente per caricare i dati...</option>
                </select>
                <button type="button" id="saveClientButton" title="Salva i dati del cliente e del contratto" class="bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-4 rounded-lg shadow-md transition-transform transform hover:scale-105" style="width: auto; margin-top: 0;">SALVA CLIENTE</button>
                <button type="button" id="deleteClientButton" title="Elimina il cliente selezionato" class="bg-red-600 hover:bg-red-700 text-white font-semibold py-2 px-4 rounded-lg shadow-md transition-transform transform hover:scale-105" style="width: auto; margin-top: 0;">ELIMINA</button>
            </div>
        </div>
        <div class="flex justify-center my-4">
            <button type="button" id="force-sync" class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-lg shadow-md transition-transform transform hover:scale-105 mr-4" style="width: auto; margin-top: 0;">
                AGGIORNA DATI
            </button>
            <button type="button" id="clearOrderButton" class="bg-yellow-500 hover:bg-yellow-600 text-white font-semibold py-2 px-4 rounded-lg shadow-md transition-transform transform hover:scale-105" style="width: auto; margin-top: 0;">
                PULISCI CAMPI
            </button>
        </div>
        <form id="orderForm">
            ...
            <button type="submit" id="submitButton"class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-4 rounded-lg shadow-md uppercase w-full mt-8 transition-transform transform hover:scale-105 text-lg">
...
// ==========================================================
// FUNZIONI DI GENERAZIONE HTML (Invariate)
// ... (tutte le funzioni da generateColtelliHtml a generateContractHtml restano qui, identiche a prima) ...
// ==========================================================
...
<tbody>
<tr>
<td class="border p-2 text-left uppercase">Coltelleria linea <input type ="text" id="linea_coltelleria_1" name="linea_coltelleria_1" class="contratto-input w-32"></td>
...
<td class="border p-2 uppercase">Mensile</td>
</tr>
<tr>
<td class="border p-2 text-left uppercase">Coltelleria linea <input type ="text" id="linea_coltelleria_2" name="linea_coltelleria_2" class="contratto-input w-32"></td>
...
            <div class="text-center"><p class="font-bold">PROPONENTE</p><p>COLTELLERIA RAGUSA S.R.L.</p>
            <p> CONTRADA RIZZA SNC - ZONA PIP 90033 CHIUSA SCLAFANI (PA)</p>
            <p> TEL: +39-091-835-3641 MAIL: INFO@COLTELLOSICILIANO.IT</p>
            <p> P.IVA: 06680510820 SITO WEB: WWW.COLTELLOSICILIANO.IT </p> </div>
...
// ==========================================================
// <<-- NUOVE FUNZIONI PER LA GESTIONE PROFILI CLIENTE
// ==========================================================
async function populateClientSelector() {
    const selector = document.getElementById('client-selector');
    selector.innerHTML = '<option value="">Seleziona un cliente per caricare i dati...</option>';
    try {
        const db = await openDB();
        ...
        console.error("Errore nel popolare il selettore clienti:", e);
    }
}
...
async function saveClientProfile() {
    ...
    const profileData = {
        ...
        // Dati Contratto (aggiungi tutti i campi che servono)
        contratto_data: document.getElementById('contratto_data').value,
        contratto_operatore: document.getElementById('contratto_operatore').value,
        ...
        contratto_luogo_nascita: document.getElementById('contratto_luogo_nascita').value,
        contratto_residenza_comune: document.getElementById('contratto_residenza_comune').value,
        contratto_residenza_provincia: document.getElementById('contratto_residenza_provincia').value,
        contratto_residenza_via: document.getElementById('contratto_residenza_via').value,
        contratto_domicilio_via: document.getElementById('contratto_domicilio_via').value,
        contratto_domicilio_comune: document.getElementById('contratto_domicilio_comune').value,
        contratto_domicilio_provincia: document.getElementById('contratto_domicilio_provincia').value,
        ...
        // ... e così via per tutti gli altri campi del contratto
    };
    ...
    try {
        ...
        document.getElementById('client-selector').value = clientName.trim();
        showMessage(`Profilo "${clientName.trim()}" salvato con successo!`, 'success');
    } catch (e) {
        console.error("Errore nel salvataggio del profilo:", e);
        showMessage("Errore durante il salvataggio del profilo.", "error");
    }
}
...
async function loadClientProfile() {
    ...
    try {
        ...
        if (profileData) {
            ...
            document.getElementById('contratto_data').value = profileData.contratto_data || '';
            document.getElementById('contratto_operatore').value = profileData.contratto_operatore || '';
            ...
            document.getElementById('contratto_data_nascita').value = profileData.contratto_data_nascita || '';
            document.getElementById('contratto_luogo_nascita').value = profileData.contratto_luogo_nascita || '';
            document.getElementById('contratto_residenza_comune').value = profileData.contratto_residenza_comune || '';
            document.getElementById('contratto_residenza_provincia').value = profileData.contratto_residenza_provincia || '';
            document.getElementById('contratto_residenza_via').value = profileData.contratto_residenza_via || '';
            document.getElementById('contratto_domicilio_via').value = profileData.contratto_domicilio_via || '';
            document.getElementById('contratto_domicilio_comune').value = profileData.contratto_domicilio_comune || '';
            document.getElementById('contratto_domicilio_provincia').value = profileData.contratto_domicilio_provincia || '';
            document.getElementById('contratto_partita_iva').value = profileData.contratto_partita_iva || '';
            document.getElementById('contratto_codice_fiscale').value = profileData.contratto_codice_fiscale || '';
            // ... e così via per tutti gli altri campi del contratto
        ...
        showMessage("Errore durante il caricamento del profilo.", "error");
    }
}
...
async function deleteClientProfile() {
    ...
    if (confirm(`Sei sicuro di voler eliminare il profilo di "${clientName}"? L'operazione è irreversibile.`)) {
        try {
            ...
            console.error("Errore nell'eliminazione del profilo:", e);
            showMessage("Errore durante l'eliminazione del profilo.", "error");
        }
    }
}
...
async function setupEventListeners() {
    ...
    document.getElementById('clearOrderButton').addEventListener('click', async () => {
        if (confirm('Sei sicuro di voler cancellare tutti i dati inseriti nel modulo? L\'operazione è irreversibile.')) {
            ...
            document.getElementById('client-selector').value = ''; // Deseleziona cliente
    ...
    // <<-- NUOVI EVENTI PER GESTIONE PROFILI
    ...
    // Carica lo stato del modulo e popola il selettore dopo che tutto è stato renderizzato
    await loadFormState();
    await populateClientSelector(); // <<-- Popola il menu all'avvio
}

// ... (Il resto delle funzioni da saveFormState a hideLoading resta invariato) ...
...
                    if (notesInput && notesInput.value.trim()) quantities.nota_articolo = notesInput.value.trim();
                    
                    const priceValue = row.querySelector('input[name="prezzo"]')?.value.trim();
